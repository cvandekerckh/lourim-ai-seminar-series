...
\node[tool] (rr)   {\textbf{Research Rabbit}\\\scriptsize\emph{Discovery}\\\scriptsize what relevant papers exist?};
\node[tool, right=of rr]   (el) {\textbf{Elicit}\\\scriptsize\emph{Screening}\\\scriptsize which papers should I include?};
\node[tool, right=of el]   (nb) {\textbf{NotebookLM}\\\scriptsize\emph{Synthesis}\\\scriptsize what do they collectively say?};
\node[tool, below=of nb]   (cg) {\textbf{ChatGPT Deep Res.}\\\scriptsize\emph{Gap analysis}\\\scriptsize what is still open?};
\node[tool, left=of cg]    (jn) {\textbf{Jenni AI}\\\scriptsize\emph{Drafting}\\\scriptsize how do I write it down?};
\node[tool, left=of jn]    (pp) {\textbf{Paperpal}\\\scriptsize\emph{Polishing}\\\scriptsize does it read well?};
\node[tool, below=of pp]   (gm) {\textbf{Gamma}\\\scriptsize\emph{Slides}\\\scriptsize how do I present it?};
